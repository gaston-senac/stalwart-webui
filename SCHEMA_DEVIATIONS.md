...
- **What**: injects `level` and `event` as filterable columns on the `x:Log` list, and provides URL-persisted, client-side noise exclusions for schema-advertised telemetry metric events, routine task-manager/task-queue events, and `store.blob-store-purged`. Task failures and retries remain visible. The `clientOnly` flag is declared as `ClientOnlyFilterEnum` (an intersection type), not on the official `FilterEnum` in `schema.ts`.
